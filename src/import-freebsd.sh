...
echo "Adding 2.0" 1>&2
{
	../git-massage.pl -p FreeBSD -a FreeBSD-2.0-START $ARCHIVE/freebsd.git/ --reverse --use-done-feature --progress=1000 $REF_IMPORT_END
	echo done
} | gfi
...
{
	../git-massage.pl -p FreeBSD -f FreeBSD-2.0-END $ARCHIVE/freebsd.git/ --reverse --use-done-feature --progress=1000 ^$REF_IMPORT_END $REF_REMAINING
	echo done
} | gfi
